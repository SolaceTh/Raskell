...
drawHypnoFractal :: Float -> Float -> Float -> IO ()
drawHypnoFractal forward_distance angle iterations = drawFractalLoop (0.0, 0.0) forward_distance angle iterations

drawFractalLoop :: Point -> Float -> Float -> Float -> IO ()
drawFractalLoop _ _ _ 0.0 = return ()
drawFractalLoop startPoint fd ang iter = do
    let (x1, y1) = startPoint
        (x2, y2) = (x1 + cos (ang * pi / 180.0) * fd, y1 + sin (ang * pi / 180.0) * fd)
        new_angle = ang + 10.0  -- Increment angle by 10 degrees
        new_fd = fd + 10.0  -- Increment forward distance by 10
        new_iter = iter - 1.0
